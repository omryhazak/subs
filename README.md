...
<p align="center">
  <strong>Live panels for every Claude Code sub-agent.</strong><br />
  Stop guessing what your agents are doing. Watch them work in real time.
</p>

<p align="center">
  <a href="https://marketplace.visualstudio.com/items?itemName=OmryHazak.subs"><img src="https://img.shields.io/visual-studio-marketplace/v/OmryHazak.subs?style=flat-square" alt="Marketplace Version" /></a>
  <a href="https://marketplace.visualstudio.com/items?itemName=OmryHazak.subs"><img src="https://img.shields.io/visual-studio-marketplace/i/OmryHazak.subs?style=flat-square" alt="Installs" /></a>
  <a href="LICENSE"><img src="https://img.shields.io/github/license/omryhazak/subs?style=flat-square" alt="License" /></a>
  <a href="https://github.com/omryhazak/subs/actions/workflows/ci.yml"><img src="https://img.shields.io/github/actions/workflow/status/omryhazak/subs/ci.yml?style=flat-square&label=CI" alt="CI" /></a>
</p>

---

When Claude Code spawns parallel agents, they work invisibly. You wait. You hope. You get a summary at the end.

**Subs cracks open the black box.** Every sub-agent gets its own live panel, streaming tool calls, reasoning, and results as they happen.
...
- **Live streaming** -- tool calls, assistant text, and results appear in real time
...
Claude Code writes sub-agent transcripts as JSONL files in `/tmp/claude-{UID}/`. Subs watches for new `.output` files, opens a webview panel for each one, and incrementally parses new bytes every 400ms. When no new data appears for 5 seconds, the agent is marked as done.
...
<details>
<summary>Which Claude Code versions are supported?</summary>

Any version that writes sub-agent transcripts to `/tmp/claude-{UID}/`. This has been the standard output location since Claude Code's launch.
...
No. Subs makes zero network requests and collects no telemetry. It only reads local JSONL files from your `/tmp/` directory.
